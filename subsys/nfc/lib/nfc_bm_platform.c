...
NRF_SDH_SOC_OBSERVER(nfc_sdh_soc, on_soc_evt, NULL, 0);
...
nrfx_err_t nfc_platform_setup(nfc_lib_cb_resolve_t nfc_lib_cb_resolve, uint8_t *p_irq_priority)
{
	...
	IRQ_DIRECT_CONNECT(DT_IRQN(NFCT), CONFIG_NFCT_IRQ_PRIORITY,
			   nfc_isr_wrapper, 0);
...
#if IS_ENABLED(CONFIG_SOFTDEVICE)
		/* We need to start HFCLK. This must not be done form IRQ
		 * priorities reserved by the SoftDevice.
		 */
		sd_res = sd_clock_hfclk_request();
		LOG_DBG("sd_clock_hfclk_request returned %u", sd_res);
...
#error "No supported clock control"
	...
	case NRFX_NFCT_EVT_FIELD_LOST:
		...
		LOG_DBG("sd_clock_hfclk_release returned %u", sd_res);
		__ASSERT_NO_MSG(sd_res == NRF_SUCCESS);
#else
#error "No supported clock control"
#endif
